...
export default function BlogDetails() {
    ...
    const authorAvatar = blog.author?.profileImage
        ...
        : `https://ui-avatars.com/api/?name=${encodeURIComponent(authorName)}&background=f1f5f9&color=4f46e5&bold=true`;
        ...
        <div className="text-gray-800 antialiased min-h-screen flex flex-col bg-white">
            ...
            <main className="flex-grow pt-28 pb-24">
                    ...
                    <div className="flex items-center justify-between pb-6 border-b border-slate-100">
                        ...
                        <div className="flex gap-2.5">
                            ...
                            <button
                                ...
                                className="w-8 h-8 rounded-full border border-slate-200 hover:border-slate-400 hover:bg-slate-50 flex items-center justify-center text-slate-500 hover:text-slate-800 transition-all relative"
                                title="Copy Link"
                            >
                ...
                <div className="max-w-5xl mx-auto px-0 sm:px-6 mb-16">
                    <div className="aspect-[21/9] w-full overflow-hidden sm:rounded-3xl shadow-sm bg-slate-100">
                        <img
                            src={imageUrl}
                            alt={blog.title}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <aside className="hidden lg:block lg:col-span-3">
                            <div className="sticky top-28 space-y-5 text-left">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                    <ShareNetwork size={14} />
                                    Table of Contents
                                </p>
                                <nav className="space-y-3 border-l border-slate-100 pl-4 py-1">
                                    {headings.map((h, idx) => (
                                        <a
                                            key={idx}
                                            href={`#${h.id}`}
                                            className="block text-xs font-bold text-slate-500 hover:text-indigo-600 transition-colors leading-tight"
                                        >
                                            {h.text}
                                        </a>
                                    ))}
                        ...
                        <article className="lg:col-span-6 prose max-w-none text-left">
                            ...
                            {/* Tag pill list */}
                            {blog.tags && (Array.isArray(blog.tags) ? blog.tags : blog.tags.split(',')).length > 0 && (
                                <div className="mt-12 pt-8 border-t border-slate-100 flex flex-wrap gap-2">
                                    {(Array.isArray(blog.tags) ? blog.tags : blog.tags.split(',')).map((tag, idx) => (
                                        <span
                                            key={idx}
                                            className="px-3.5 py-1.5 bg-slate-50 text-slate-600 rounded-full text-xs font-bold border border-slate-100 hover:bg-slate-100 hover:text-slate-800 transition-all cursor-pointer"
                                        >
                                            #{tag.trim()}
                                        </span>
                                    ))}
                        ...
                        <aside className="hidden lg:block lg:col-span-3">
                            <div className="sticky top-28 bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-sm text-left">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Author Profile</p>
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        />
                                        <div>
                                            <h4 className="text-sm font-bold text-slate-900 leading-tight">{authorName}</h4>
                                            <p className="text-[10px] text-slate-400 font-semibold leading-none mt-1">Publisher</p>
                                        </div>
                                    </div>
                ...
                <section className="max-w-4xl mx-auto px-4 sm:px-6 mt-20 pt-16 border-t border-slate-100 text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
                        ...
                        <div className="md:col-span-5">
                            <form onSubmit={handleCommentSubmit} className="space-y-4 bg-slate-50/50 p-5 rounded-3xl border border-slate-100">
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide">Add a comment</p>
                                <div className="space-y-1">
                                    <input
                                        ...
                                        className="w-full text-xs px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-800 font-semibold"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <textarea
                                        ...
                                        className="w-full text-xs px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-800 leading-relaxed resize-none font-medium"
                                    />
                                </div>
                                <button type="submit" className="w-full btn-primary py-2.5 text-xs font-semibold shadow-sm flex items-center justify-center gap-2">
                        ...
                        <div className="md:col-span-7 space-y-6">
                            {comments.map((c, idx) => (
                                <div key={idx} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-2.5">
                                    <div className="flex justify-between items-center">
                                        <div className="flex items-center gap-2.5">
                                            <div className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-600 ring-2 ring-white">
                                                {c.name[0]}
                                            </div>
